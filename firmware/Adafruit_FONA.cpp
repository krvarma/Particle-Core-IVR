...
int8_t Adafruit_FONA::getFMSignalLevel(uint16_t station) {
    ...
    char *p = strstr(replybuffer, PSTR("+FMSIGNAL: "));
    if (p == 0) return -1;
    p+=11;
...
bool Adafruit_FONA::PWM(uint16_t period, uint8_t duty) {
...
int Adafruit_FONA::getStatus(){
    getReply(F("AT+CLCC"));
    
    if(strncmp(replybuffer, "OK", 2) == 0){
        return -1;
    }
	...
	ptr = strchr((const char *)replybuffer, ',');
	if (ptr == NULL)
		return -1;
	start = strchr(ptr + 1, ',');
	if (start == NULL)
    ...
    if(strncmp(replybuffer, "+CLCC", 5) == 0){
	    sscanf(&replybuffer[7], "%d,%d,%d", &cid, &dir, &status);
    }
...
bool Adafruit_FONA::parseReply(const char *toreply,
          uint16_t *v, char divider, uint8_t index) {
  char *p = strstr(replybuffer, (prog_char*)toreply);  // get the pointer to the voltage
  if (p == 0) return false;
...
// response.
bool Adafruit_FONA::parseReplyQuoted(const char *toreply,
